...
| `WorkflowVersioningService` | `WorkflowVersioningService.php` | Publish (runs verification first) + version history + rollback. Extends `BaseService`; uses `DB::transaction()` closures internally (deviates from the controller-owns-transactions convention other modules follow — see repo-level CLAUDE.md). Key methods: `publish()` creates a new version from the draft, `listVersions()` returns all versions descending, `showVersion()` returns a single version with its definition, `compareVersions()` computes a structural diff between two versions (nodes/edges added/removed/modified, trigger/settings changed), `rollback()` restores a previous version's definition as a **new** version (preserves full audit trail via `rollback_source_version_id`). Rollback re-runs verification before creating the new version to catch definitions that are no longer publishable. Does **not** overwrite `draft_definition` — the user's in-progress draft is preserved. |
...
| `AiWorkflowGeneratorService` | `Ai/AiWorkflowGeneratorService.php` | Thin HTTP client (mirrors `Modules\KnowledgeBase\Services\RagService`'s pattern, same `services.rag` config/`X-API-Key` header) to the external RAG service's `POST /api/v1/ai/workflows/generate`. Used only by `WorkflowController::generateWithAi()` — nothing persists here; see `POST /ai-generate` below. |
| `RagAiClient` | `Ai/RagAiClient.php` | Runtime HTTP client for the `ai-generator`/`ai-classifier` node executors — `POST /api/v1/ai/generate` and `POST /api/v1/ai/classify` on the RAG service (same `services.rag` config/`X-API-Key` header as the services above). Implements both `Contracts/AiContentGenerator` and `Contracts/AiTextClassifier`, bound to those interfaces in `WorkflowsServiceProvider`. Both RAG endpoints return HTTP 200 even on a "soft failure" (`success: false`) — the client treats that the same as a transient error (throws `Exceptions/AiServiceException` with `retryable: true`), while a hard HTTP failure is retryable only for 5xx/429. See `docs/RAG_AI_NODES_API.md` for the wire contract. |
...
| Executors (`Executors/`, implement `Contracts/NodeExecutor`) | one file per node type | `ManualTriggerExecutor`, `FormTriggerExecutor`, `WebhookTriggerExecutor`, `IfNodeExecutor`, `SwitchNodeExecutor`, `ForkNodeExecutor`, `MergeNodeExecutor`, `TerminationNodeExecutor`, `SendEmailExecutor`, `ParseJsonExecutor`, `ClickUpCreateTaskExecutor`, `HubSpotCreateContactExecutor`, `HubSpotCreateDealExecutor`, `TaskNodeExecutor`, `SubWorkflowExecutor`, `DynamicFlowExecutor`, `DynamicEntryExecutor`, `AiGeneratorExecutor`, `AiClassifierExecutor` — all registered in `WorkflowsServiceProvider`. Each returns a `NodeExecutionResult` tagged with a `ResultKind` (Proceed/Branch/Wait/Fail/Terminate/Noop). `AiGeneratorExecutor`/`AiClassifierExecutor` catch `Exceptions/AiServiceException` from their injected `Contracts/AiContentGenerator`/`Contracts/AiTextClassifier` (bound to `Ai/RagAiClient`) and map it to `NodeExecutionResult::fail(..., retryable: $e->isRetryable())`. |
...
| AI | `ai-generator` | AI Generator | Generates text via the external RAG service (`POST /api/v1/ai/generate`, see `docs/RAG_AI_NODES_API.md`), optionally grounded in knowledge-base documents (`knowledgeBaseDocuments`, forwarded as RAG's `article_ids`) and a free-text `tone`. Writes the generated `content` string to `outputVariable` (default `ai_output`). See [`Modules/KnowledgeBase/docs/README.md`](../../KnowledgeBase/docs/README.md) |
| AI | `ai-classifier` | AI Classifier | Classifies `text` into exactly one of at least 2 `categories` via the external RAG service (`POST /api/v1/ai/classify`, see `docs/RAG_AI_NODES_API.md`). Writes `{classification, confidence}` as a single object to `outputVariable` (default `ai_classification`) — a downstream `if-node`/`switch` can branch on `{{context.<outputVariable>.classification}}` or `.confidence` (dot-path resolution via `data_get`, same as any other nested context read). |
...
| Action | `hubspot-create-contact` | HubSpot: Create Contact | Creates a contact in the connected HubSpot account via `firstName`/`lastName`/`email`/`phone` config fields (all optional individually, but at least one of first name/last name/email must be set — HubSpot's own minimum). See [`Modules/Integrations/docs/README.md`](../../Integrations/docs/README.md). Note: the frontend has unrelated, unwired mock schemas for different node type strings `hubspot-contact`/`hubspot-deal` — don't confuse the two. |
| Action | `hubspot-create-deal` | HubSpot: Create Deal | Creates a deal in the connected HubSpot account. Required: `dealName`, `dealStage` (HubSpot's internal stage ID — tenant-specific, no picker built yet, plain text like ClickUp's `listId`). Optional: `pipeline` (internal ID), `amount`, `closeDate` (ISO 8601), `ownerId`, and `contactId` — a single flat `{{context.<key>}}` value (nested paths like `{{context.someNode.someField}}` are **not** valid per the shared template-variable syntax; see `Modules/Workflows/app/Services/Verification/Rules/NodeType/Concerns/VariableAvailability.php`) that, if numeric after rendering, associates the new deal with an existing HubSpot contact (association type ID `3`, Deal→Contact). See [`Modules/Integrations/docs/README.md`](../../Integrations/docs/README.md) for `HubSpotClient::createDeal()`. Same `hubspot-deal` frontend-mock collision caveat as above applies. |
...
- `POST /ai-generate` (Manager/BusinessOwner only) asks the external RAG service to draft a workflow definition. Client sends `prompt`, `description`, `workflow_name`, and `team_id` — **all four required**. Only `prompt` (≥5 chars trimmed — matches RAG's own `AIWorkflowRequest` rule exactly), `workflow_name`, and a server-set `tenant_id` are forwarded to RAG — RAG's schema has no `description`/`goal`/`team_id` field. `description` and `team_id` are **plain workflow metadata**, never sent to RAG: `description` is the same field/limit as `StoreWorkflowRequest`'s (method=blank/template), just carried through the preview — the client's own text wins verbatim in the response, with RAG's AI-generated `workflow.description` only used as a fallback if somehow absent; `team_id` is purely echoed back so the client can carry it into the subsequent create call. The controller always adds `tenant_id` (RAG's own field name) from the authenticated user — never client-supplied — so RAG scopes its document-grounding search to that tenant only. Nothing is persisted; the response's `data` object is shaped like `WorkflowTemplateResource` (`id: null, name, description, category: null, is_global: false, usage_count: 0`) plus `definition` and the echoed `team_id`, with `success`/`validation`/`ai`/`context_documents_used` alongside — feed `data.definition` (+ `data.team_id`, `data.description`) into `POST /` with `method: 'ai_confirmed'` to actually create the workflow. A RAG-side HTTP failure surfaces as `502`; a RAG-side "soft failure" (bad LLM output, its own validation failing, or the LLM provider itself erroring) still returns `200` with `success: false` and `ai.provider: "fallback"` — callers must check `success`, not just the HTTP status. **Known gap on the RAG service side (not this app):** its `X-API-Key` check currently accepts every request regardless of key correctness or presence — verified live (missing key and a garbage key both return `200`, not `401`) — so `RAG_SERVICE_API_KEY` is not actually enforced upstream right now.
...
- **KnowledgeBase**: `ai-generator` node's `knowledgeBaseDocuments` field (KnowledgeBase document IDs) is forwarded by `Ai/RagAiClient` as RAG's `article_ids` — see `Modules/KnowledgeBase/docs/README.md`.
- **Integrations**: `send-email` node sends through Integrations' connected Gmail workspace; `clickup-create-task` node creates a task through Integrations' connected ClickUp workspace (`ClickUpClient`); `hubspot-create-contact`/`hubspot-create-deal` nodes create a contact/deal through Integrations' connected HubSpot account (`HubSpotClient`) — see `Modules/Integrations/docs/README.md`.
...
- **Dynamic-flow submit/resume wiring** (`DynamicFlowController::storeDefinition` + `DynamicFlowExecutor` + `NodeRunner::resolvePlan` + `WorkflowExecutionEngine::onWait`): (1) the controller sets `definition.trigger = { type: 'dynamic-entry' }` on the submitted segment — `WorkflowDispatcher::dispatch()` needs a trigger node to seed the child's first execution, and the segment's entry node is always `dynamic-entry`. (2) The controller moves the `WorkflowDynamicFlow` row to `executing` and the parent instance to `running` **before** dispatching the child, because the child can finish (sync queue, or a fast worker) before the request returns — if it re-enters the parent's `dynamic-flow` node while the row still says `awaiting_design` it just re-parks. (3) `DynamicFlowExecutor` and `NodeRunner::resolvePlan` resolve the child instance via the `parent_execution_id` link (written atomically at dispatch), not `child_instance_id` (backfilled only after dispatch returns). (4) `DynamicFlowExecutor::handleInitialEntry` deliberately sets the parent instance to `paused` (`paused_reason = 'dynamic_flow:awaiting_design'`); `onWait` must **not** override an already-`paused` instance with `waiting` — `InstanceLifecycleManager` has no `paused → waiting` transition and it would throw.
...
- `ai-generator` and `ai-classifier` are both fully wired end-to-end: seeded (`NodeDefinitionSeeder`), verified (`AiGeneratorNodeTypeRule`/`AiClassifierNodeTypeRule`), executed (`Executors/AiGeneratorExecutor.php`/`Executors/AiClassifierExecutor.php`, registered in `WorkflowsServiceProvider`), and backed by a real RAG HTTP client (`Ai/RagAiClient.php`, bound to `Contracts/AiContentGenerator`/`Contracts/AiTextClassifier`). `ContextualVerificationRule::verifyKnowledgeBaseDocuments()` now reads the seeded `knowledgeBaseDocuments` field first, falling back to the legacy `kbDocs` key for older definitions (previously it only ever read `kbDocs`, silently no-oping against the real field — see `Modules/KnowledgeBase/docs/README.md` for background on that historical mismatch).
