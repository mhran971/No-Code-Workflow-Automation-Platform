...
class WorkflowManagementService
{
    ...
    {
        $query = Workflow::query()->visibleTo($actor);
        ...
        return $query
            ...
            ->when(
                ...
                fn (Builder $query, string $description) => $query->where('description', 'like', "%{$description}%")
            )
            ->when(
                $filters['team_id'] ?? null,
                fn (Builder $query, $teamId) => $query->where('team_id', (int) $teamId)
            )
            ->when(
                $filters['status'] ?? null,
    ...
    public function createWorkflow(User $actor, array $data): Workflow
    {
        ...
        if ($actor->role === Role::Manager && $team = $actor->getManagedTeam() === null) {
    ...
    public function updateDraft(User $actor, Workflow $workflow, array $data): array
    {
        ...
        // Check if the draft revision matches the expected revision to prevent overwriting changes
        if ((int) $workflow->draft_revision !== (int) $data['expected_draft_revision']) {
