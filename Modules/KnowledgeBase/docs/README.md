...
Tenant-scoped PDF document library (metadata + file storage, tagging, and a fixed document-type taxonomy). It has **no text-extraction, chunking, or embedding pipeline** — documents are stored as opaque PDF binaries plus metadata; any "knowledge retrieval" for AI features is not implemented at this layer yet (see Cross-Module Dependencies).

## Data Model

| Model | Key fields/relationships | Migration file |
| --- | --- | --- |
| `Document` (`app/Models/Document.php`) | `tenant_id`, `title`, `document_type_id`, `file_path`, `is_active` (bool, default `true`); `belongsTo` `Tenant` (`Modules\Auth\Models\Tenant`), `belongsTo` `DocumentType`, `belongsToMany` `Tag` via `document_tag` | `2026_02_13_120000_create_documents_table.php`; `is_active` added later in `2026_05_15_092841_add_is_active_to_documents_table.php`; a prior `uploaded_at` column was dropped in `2026_02_13_130000_remove_uploaded_at_from_documents_table.php` (API now reports `created_at` as `uploaded_at`, see `DocumentResource`) |
...
| Class | File | Purpose |
| --- | --- | --- |
| `DocumentService` | `app/Services/DocumentService.php` | Upload (stores file + creates row + syncs tags, `upload()` L31-48), metadata update (`updateMetadata()` L55-71, file is never replaced), tenant-scoped list/get, streamed download (`getFileResponse()` L90-111), active-flag toggle (`setActiveStatus()` L113-120), delete (`delete()` L122-128), private `storeFile()` (L130-139) |
...
Both services extend `App\Services\BaseService` and set `protected ?string $repositoryClass` explicitly (redundant — `BaseService`'s naming-convention guesser would resolve the same class). They follow the module convention structurally, but **neither currently relies on `__call()` delegation in practice**: every method the controllers invoke is explicitly defined on the service class itself, not magically forwarded to the repository. No controller or service in this module uses `DB::transaction()`/`beginTransaction()` — multi-step mutations (e.g. `upload()` creating a `Document` row then syncing tags) run as plain sequential Eloquent calls with no explicit transaction wrapper.
...
This module defines **no Enums** (no `app/Enums/` directory). It consumes `Modules\Auth\Enums\Role` (`TagService::getTagsForCurrentUser()` checks `$user->role === Role::Admin`) and raw role-name strings (`business_owner`, `manager`) in route middleware. It also has **no `app/Exceptions/`** and **no `app/Jobs/`** directories — all errors are inline `response()->json([...], 4xx)` calls or standard Laravel `FormRequest` validation, and document upload is fully synchronous (no queued post-processing of any kind).

## Key Business Rules & Gotchas

- **No content pipeline.** There is no text extraction, chunking, or embedding/vector storage anywhere in this module — `Document` only holds a `file_path` pointer. Confirmed by reading every model/service/migration; nothing parses PDF content.
...
- **Delete leaves an orphaned file.** `DocumentService::delete()` (L122-128) only does `$document->delete()` — it never calls `Storage::delete()`, so the physical PDF is never removed. There's a literal `// TODO` there acknowledging deletion should probably be a soft (`is_active=false`) operation for audit purposes instead, and a matching `// TODO` on `setActiveStatus()` (L115) noting there's currently **no check that a live workflow still references the document** before it's deactivated.
...
**How Workflows references KnowledgeBase — verification time only, and currently broken:**

- `Modules/Workflows/app/Services/Verification/Rules/ContextualVerificationRule.php` is the **only file outside this module** that touches `Modules\KnowledgeBase` (imports `Document` at L6). Its `verifyKnowledgeBaseDocuments()` (L80-121) scans every node whose `type` starts with `ai-`, reads `node['config']['kbDocs']`, and validates each entry is numeric and exists as a `Document` row owned by the workflow's tenant (L111-114).
- **Key mismatch:** the `ai-generator` node's config schema, seeded by `Modules/Workflows/database/seeders/NodeDefinitionSeeder.php` (L132), names this field `knowledgeBaseDocuments` (type `"tags"`) — that's what the builder UI actually writes into `node.config`. But the verification rule reads `config['kbDocs']`, a different key, and `WorkflowDefinitionNormalizer.php` contains no aliasing between the two (verified — neither string appears in that file). So as currently written, a workflow built through the UI populates `knowledgeBaseDocuments`, the validator only ever inspects `kbDocs` (which will be absent), and the existence/tenant-ownership check silently no-ops.
- `AiGeneratorNodeTypeRule` (the per-node-type structural rule) validates only `prompt` and `outputVariable` — it doesn't touch either `kbDocs` or `knowledgeBaseDocuments`.

**How the `ai-generator` executor actually consumes documents — it doesn't (yet):**

- `AiGeneratorExecutor::execute()` (`Modules/Workflows/app/Services/Execution/Executors/AiGeneratorExecutor.php` L25-41) renders the `prompt` template and calls `$this->ai->generate($prompt, $config)`, forwarding the *entire* raw config array (whatever KB-document field is set) into an `AiContentGenerator` interface (`.../Contracts/AiContentGenerator.php`). It never itself queries `Modules\KnowledgeBase`, fetches file content, or does any retrieval.
- `AiContentGenerator` has **no concrete implementation anywhere in the codebase** (repo-wide search for `implements AiContentGenerator` finds nothing outside `vendor/composer` autoload maps). Whatever service would turn document references into prompt context does not exist yet.
- `AiGeneratorExecutor` is also **not registered** in `NodeExecutorRegistry` (`Modules/Workflows/app/Providers/WorkflowsServiceProvider.php`, executor-registration block L144-164 — compare to `TaskNodeExecutor`/`SendEmailExecutor`, which are registered there; `AiGeneratorExecutor` is absent and not even imported). If a published workflow instance reaches an `ai-generator` node at runtime, `NodeExecutorRegistry::for('ai-generator')` throws `UnsupportedNodeTypeException` (`.../Execution/NodeExecutorRegistry.php` L38).

**Net state:** the AI Generator node is fully defined for the builder UI (`NodeDefinitionSeeder`) and partially wired for validation (`ContextualVerificationRule`, though its document-ID check is dead due to the key mismatch above), but it is **not wired for execution at all**, and there is no working document-content retrieval/RAG pipeline in either module yet.
