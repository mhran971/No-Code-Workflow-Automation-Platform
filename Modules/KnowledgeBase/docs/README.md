...
**Net state:** the AI Generator node is fully defined for the builder UI (`NodeDefinitionSeeder`) and partially wired for validation (`ContextualVerificationRule`, though its document-ID check is dead due to the key mismatch above), but it is **not wired for execution at all**. Note this is a distinct gap from the [RAG Ingestion Pipeline](#rag-ingestion-pipeline) above: documents *are* now pushed into the external RAG service's vector index on upload, but nothing in `Workflows` queries that index (or otherwise retrieves document content) to build an `ai-generator` prompt — the ingestion and retrieval halves of RAG remain unconnected.
