...
class WorkflowVerificationTest extends TestCase
{
    ...
    // ── 2f. Parallel join metadata ───────────────────────────────────────────

    /** @test */
    public function test_graph_fork_join_edge_without_join_node_key_produces_join_missing_error(): void
    {
        $definition = $this->validDefinition([
            'nodes' => [
                ['id' => 'n1',    'type' => 'parallel-split', 'config' => [], 'is_entry_point' => true],
                ['id' => 'task_a','type' => 'send-email',      'config' => []],
                ['id' => 'join',  'type' => 'parallel-join',   'config' => [], 'is_terminal' => true],
            ],
            'edges' => [
                [
                    'source'            => 'n1',
                    ...
                    // join_node_key intentionally omitted
                ],
        ...
        $this->assertNotPublishable($result);
        $this->assertHasError($result, 'parallel.join_missing');
    }

    /** @test */
    public function test_graph_fork_join_edge_referencing_nonexistent_join_node_produces_join_unknown_error(): void
    {
        $definition = $this->validDefinition([
            'nodes' => [
                ['id' => 'n1',    'type' => 'parallel-split', 'config' => [], 'is_entry_point' => true],
        ...
        $this->assertNotPublishable($result);
        $this->assertHasError($result, 'parallel.join_unknown');
    }

    /** @test */
    public function test_graph_fork_join_edge_referencing_wrong_node_type_as_join_produces_join_type_invalid_error(): void
    {
        $definition = $this->validDefinition([
            'nodes' => [
                ['id' => 'n1',      'type' => 'parallel-split', 'config' => [], 'is_entry_point' => true],
                ['id' => 'task_a',  'type' => 'send-email',      'config' => []],
                ['id' => 'not_join','type' => 'send-email',      'config' => [], 'is_terminal' => true], // wrong type
            ],
            'edges' => [
                [
                    'source'            => 'n1',
                    ...
                    'branch_type'       => 'parallel',
                    'parallel_strategy' => 'fork_join',
                    'join_node_key'     => 'not_join',
                ],
                ['source' => 'task_a', 'target' => 'not_join'],
            ],
        ]);
        ...
        $this->assertNotPublishable($result);
        $this->assertHasError($result, 'parallel.join_type_invalid');
    }

    /** @test */
    public function test_graph_fire_and_forget_parallel_edge_without_join_node_passes(): void
    {
        $definition = $this->validDefinition([
            'nodes' => [
                ['id' => 'n1',    'type' => 'parallel-split', 'config' => [], 'is_entry_point' => true],
                ['id' => 'task_a','type' => 'send-email',      'config' => [], 'is_terminal' => true],
            ],
            'edges' => [
                [
                    'source'            => 'n1',
                    'target'            => 'task_a',
                    'branch_type'       => 'parallel',
                    'parallel_strategy' => 'fire_and_forget',
                    // no join_node_key needed
                ],
            ],
        ]);
        ...
        $this->assertNoError($result, 'parallel.join_missing');
    }
