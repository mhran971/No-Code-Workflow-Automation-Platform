...
// Fixed pixel heights — input handles must align precisely with their branch rows.
const HEADER_H = 44;
const ROW_H = 28;
const EMPTY_H = 36;
const BOTTOM_PAD = 8;
...
function MergeNodeComponent({ id, data, selected }: NodeProps) {
  ...
  const rawCount = Number(d.config?.branchCount);
  const branchCount = Number.isFinite(rawCount) && rawCount > 0 ? Math.floor(rawCount) : 0;
  const branches = Array.from({ length: branchCount }, (_, i) => i);

  const hasBranches = branches.length > 0;
  const mode = typeof d.config?.mergeMode === 'string' ? d.config.mergeMode : 'parallel';
  ...
  return (
    ...
    >
      ...
      {/* Single output handle — vertically centered */}
      <Handle
        type="source"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-[10px] text-muted-foreground truncate block">{d.name || `${mode} merge`}</span>
        </div>
        {hasErrors && <AlertCircle className="h-3.5 w-3.5 text-destructive flex-shrink-0" />}
      </div>

      {/* Incoming branch rows — one dynamic input (target) handle per declared branch */}
      {hasBranches ? (
        branches.map((i) => {
          ...
          return (
            <div key={i}>
              ...
              >
                <span className="text-[10px] text-foreground/80 truncate pl-2">Branch {i + 1}</span>
              </div>
              <Handle
                type="target"
        ...
        >
          <p className="text-[10px] text-muted-foreground italic">Set the number of incoming branches</p>
        </div>
      )}
    </div>
  );
